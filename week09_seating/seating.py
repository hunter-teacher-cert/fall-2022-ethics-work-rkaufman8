...
get_avail_seats(plane,economy_sold):
    ...
    Notes: this loops over the plane and counts the number of seatsthat are "avail" or "win" and removes the number of economy_sold seats
...
seat_economy(plane,economy_sold,name):
    Similar to purchase_economy_plus but just randomy assigns a random seat.
...
main(): The main driver program - start here
"""
import random
...
def get_avail_seats(plane,economy_sold):
    ...
    Returns: the number of unsold seats
    Notes: this loops over the plane and counts the number of seats that are "avail" or "win" and removes the number of economy_sold seats
    """
    avail = 0;
    for r in plane:
        for c in r:
...
def fill_plane(plane):
    """
    Params: plane - a list of lists representing a plane comments interspersed in the code
    ...
    while total_seats > 1:
        ...
        if r > 50: #this is now irrelevant, i think
            ...
            economy_sold = purchase_economy_block(plane,economy_sold,2,"f-%d"%f_number)
            f_number = f_number + 1    
        else:
